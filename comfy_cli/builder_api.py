...
class BuilderClient:
    ...
    def create_release(self, distribution_id: str, targets: list[dict] | None = None) -> tuple[str, str]:
        """Freeze the definition and enqueue a build for ``targets``. Returns
        (versionId, statusUrl).
    ...
    def get_release(self, version_id: str) -> dict:
        """Poll a release's build status."""
    ...
    def list_builds(self) -> list[dict]:
        """GET every cursor page from /v1/builds."""
        builds: list[dict] = []
        cursor: str | None = None
        while True:
            page = self._get(("builds",), {"cursor": cursor})
            builds.extend(page.get("builds", []))
            cursor = page.get("nextCursor")
            if not cursor:
                return builds

    def get_build(self, distribution_id: str) -> dict:
        """GET /v1/builds/{id} -> a build with its full definition."""
        return self._get(("builds", distribution_id))

    def list_releases(self, distribution_id: str) -> list[dict]:
        """GET every cursor page from /v1/builds/{id}/versions."""
        releases: list[dict] = []
        cursor: str | None = None
        while True:
            page = self._get(("builds", distribution_id, "versions"), {"cursor": cursor})
            releases.extend(page.get("versions", []))
            cursor = page.get("nextCursor")
            if not cursor:
                return releases

    def get_release_logs(self, version_id: str, *, os: str | None = None, gpu: str | None = None) -> dict:
    ...
    def delete_build(self, distribution_id: str) -> None:
    ...
    def validate_build(self, distribution_id: str) -> dict:
    ...
    def get_release_manifest(self, version_id: str) -> dict:
        """GET /v1/build-versions/{id}/manifest -> the release's models and
        runtime policies."""
