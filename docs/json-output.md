...
| `cloud_rate_limited` | The cloud API throttled the request (HTTP 429) on submit or while polling — the workflow was not judged invalid; wait and retry it unchanged | same as `cloud_http_error`, plus `retry_after` (seconds) when the server sent `Retry-After` | 1 |
