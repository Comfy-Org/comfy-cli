...
class _MultipartBody:
    ...
    def __iter__(self) -> Iterator[bytes]:
        yield self.prefix
        with self.local_path.open("rb") as file:
            while chunk := file.read(UPLOAD_CHUNK_BYTES):
                yield chunk
        yield self.suffix
...
def _multipart_request(
    ...
    body = _MultipartBody(request.local_path, prefix, suffix, file_size)
...
class DeployAssetClient:
    ...
    def _resolve_miss(self, request: AssetResolveRequest, expected_hash: str) -> AssetResolveResult:
        ...
        upload, file_size = _multipart_request(self.target, request, expected_hash)
        try:
