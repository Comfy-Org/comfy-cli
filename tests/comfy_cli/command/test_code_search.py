                ...
                "results": {
                    ...
                    "results": [
                        ...
                        },
                        {
                            ...
                            "lineMatches": [
                                {
                                    "preview": "from nodes import LoadImage",
                                    "lineNumber": 9,
                                    "offsetAndLengths": [[20, 9]],
                                },
                            ],
                        },
                    ],
...
def empty_api_response():
    """An API response with no results."""
    return {
        "data": {
            "search": {
...
@pytest.fixture
def limit_hit_response(raw_api_response):
    """An API response where the limit was hit."""
    raw_api_response["data"]["search"]["results"]["limitHit"] = True
...
class TestBuildQuery:
    ...
    def test_with_repo_short_name(self):
        ...
        assert result == "repo:Comfy-Org/ComfyUI LoadImage"
...
class TestFormatResults:
    def test_formats_valid_results(self, raw_api_response):
        results = _format_results(raw_api_response)
    ...
    def test_empty_results(self, empty_api_response):
        results = _format_results(empty_api_response)
        assert results == []
        ...
        data = {
            "data": {
                "search": {
                    "results": {
                        "results": [
                            {"__typename": "FileMatch", "repository": None, "file": {"path": "x.py"}},
                        ]
                    }
                }
            }
        }
        assert _format_results(data) == []
                ...
                "search": {
                    "results": {
                        "results": [
                            {
                                "__typename": "FileMatch",
                                "repository": {"name": "github.com/Comfy-Org/ComfyUI"},
                                "file": None,
                            },
                        ]
                    }
...
class TestGetStats:
    ...
    def test_limit_hit(self, limit_hit_response):
        stats = _get_stats(limit_hit_response)
...
class TestPrintResults:
    ...
    def test_formatted_output_contains_file_info(self, capsys, raw_api_response):
        results = _format_results(raw_api_response)
        stats = _get_stats(raw_api_response)
    ...
    def test_limit_hit_message(self, capsys, limit_hit_response):
        results = _format_results(limit_hit_response)
        stats = _get_stats(limit_hit_response)
        _print_results(results, stats, json_output=False)
...
class TestCodeSearchCLI:
    ...
    def test_basic_search(self, mock_fetch, raw_api_response):
        ...
        mock_fetch.assert_called_once_with("LoadImage")
    ...
    def test_search_with_repo(self, mock_fetch, raw_api_response):
        ...
        assert result.exit_code == 0
        mock_fetch.assert_called_once_with("repo:Comfy-Org/ComfyUI LoadImage")
