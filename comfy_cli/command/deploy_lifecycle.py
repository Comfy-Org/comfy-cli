...
def run_scale(request: ScaleRequest) -> None:
    def scale(renderer: Renderer, client: DeploymentLifecycleClient, deployment_id: str) -> None:
        ...
        merged: JsonObject = {
            ...
            "min": request.minimum if request.minimum is not None else current["min"],
            "max": request.maximum if request.maximum is not None else current["max"],
        }
        try:
        ...
        if renderer.is_pretty():
            renderer.success(f"Scaled deployment {deployment_id} to min={merged['min']}, max={merged['max']}")
