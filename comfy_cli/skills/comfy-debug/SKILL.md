...
Throttled, not rejected — the workflow/request was never judged, so do not edit it. Wait `details.retry_after` seconds (when present; otherwise a few seconds) and re-run the same command unchanged.
