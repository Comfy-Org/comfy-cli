...
import jsonschema
import pytest
from deploy_up_support import FakeBuilder, deployment, option_names
...
def test_lifecycle_command_is_registered(command: str) -> None:
    ...
    assert "--deployment" in option_names(command)
...
def _bounds_free(deployment_id: str = "dep-1") -> JsonObject:
    """A deployment the service stored without worker bounds — what a web-UI or
    direct-API create produces, since the create handler writes `min`/`max` only
