...
import jsonschema
import pytest
from deploy_up_support import FakeBuilder, option_names, write_spec
...
def test_deploy_ls_is_a_registered_real_command() -> None:
    ...
    assert {"--all", "--workspace", "--status", "--limit"} <= option_names("ls")
...
def test_ls_accepts_a_live_row_that_omits_the_optional_keys(tmp_path: Path, monkeypatch) -> None:
    """The Go handler tags computeConfig / deletedAt / deletedBy / endpointUrl
    `omitempty`, so a live deployment that never carried them sends no key at
    all rather than a null. `deploy ls` emits the server rows verbatim, so a
    `required` entry for any of them would reject the ordinary case."""
    # Given
    row: JsonObject = {
        "createdAt": "2026-08-23T12:00:00Z",
        "id": "build-live",
    ...
    schema = json.loads(
        (Path(__file__).parent.parent.parent.parent / "comfy_cli" / "schemas" / "deploy_ls.json").read_text(
            encoding="utf-8"
        )
    )
    jsonschema.Draft202012Validator(schema).validate(_payload(result))
    assert [item["id"] for item in _deployments(result)] == ["build-live"]


def test_ls_still_refuses_a_row_with_no_release(tmp_path: Path, monkeypatch) -> None:
    """`releaseId` is what scopes the list to this Build, so unlike the optional
    keys above it stays required — a row without it cannot be placed."""
    # Given
    row: JsonObject = {"id": "build-live", "status": "ready"}
    _install_clients(monkeypatch, FakeBuilder(), PagedDeploy([[row]]))
