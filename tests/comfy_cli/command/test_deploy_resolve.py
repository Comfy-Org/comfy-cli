...
def _deployment(
    ...
    return {
        "id": deployment_id,
        "distributionVersionId": "release-1",
        "status": status,
...
def test_zero_matching_deployments_returns_none() -> None:
    # Given
    builder = RecordingBuilder(pages=[[{"id": "release-1"}]])
    other_release = {**_deployment("deployment-other"), "distributionVersionId": "release-other"}
...
def test_join_consumes_three_pages_from_each_exhaustive_collector() -> None:
    ...
    target = {**_deployment("deployment-target"), "distributionVersionId": "release-page-3"}
    deployments = RecordingDeployments(
        [
            [{**_deployment("deployment-other-1"), "distributionVersionId": "release-other-1"}],
            [{**_deployment("deployment-other-2"), "distributionVersionId": "release-other-2"}],
            [target],
        ]
    )
