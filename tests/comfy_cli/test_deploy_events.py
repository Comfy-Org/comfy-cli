...
def test_a_job_that_never_finished_is_left_for_the_caller_to_classify(monkeypatch, status) -> None:
    """A job the user canceled has no outputs to demand. Insisting on the array
    here reports `deploy_server_error` and buries the `deploy_job_canceled` that
    `deploy_run._terminal_result` raises from this very result."""
    # Given
    job = _job(status)
    del job["outputs"]
    _, _, sleep_fn = _install_wire(monkeypatch, [_http_error(501, "not_implemented")], [job])
...
def test_a_succeeded_job_without_outputs_is_still_a_server_shape_error(monkeypatch) -> None:
    # Given
    job = _job("succeeded")
    del job["outputs"]
    _, _, sleep_fn = _install_wire(monkeypatch, [_http_error(501, "not_implemented")], [job])
