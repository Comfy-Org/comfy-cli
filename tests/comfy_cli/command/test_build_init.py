...
def test_base_image_lands_in_the_definition_next_to_the_comfy_version(install: Path) -> None:
    """`baseImage` is a definition field, not spec metadata: `build push` sends
    the definition verbatim, so writing it here is what makes the flag reach the
    builder at all. It is ordered beside `baseComfyVersion` because the two
    together are the runtime the build is built on."""
    # Given / When
    result = _invoke(CliRunner(), ["--base-image", "cuda124-py312", *_success_args(install)])

    # Then
    assert result.exit_code == 0, result.output
    definition = _definition(install / "comfy-build.yaml")
    assert definition["baseImage"] == "cuda124-py312"
    keys = list(definition)
    assert keys.index("baseImage") == keys.index("baseComfyVersion") + 1


def test_omitting_base_image_writes_no_key_at_all(install: Path) -> None:
    """An absent key is what selects the catalog default. Writing an explicit
    null would pin the build to a value the builder has to reject."""
    # Given / When
    result = _invoke(CliRunner(), _success_args(install))

    # Then
    assert result.exit_code == 0, result.output
    assert "baseImage" not in _definition(install / "comfy-build.yaml")
