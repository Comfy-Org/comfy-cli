...
2. **`available_locally: false` means "not here", not "not curated".** The row
   ...
   download URLs. Unless the payload's `local_check` is `ok`, no files were
   checked. A pick carrying its own `local_check` was not checked either, so it
   is not yet the runnable alternative. Ask before downloading anything.
