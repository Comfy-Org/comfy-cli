...
def _reject_scan_options(
    ...
    *,
    ...
    """Refuse ``--from-snapshot`` alongside any option that only steers a scan.
    ...
    renderer.error(
        code="build_missing_input",
        message=f"--from-snapshot cannot be combined with {joined}: an imported snapshot is not a local scan.",
        hint=f"drop {joined} to import the snapshot, or drop --from-snapshot to scan the install",
        details={"conflict": ["--from-snapshot", *conflicting]},
    )
    raise typer.Exit(code=1)
...
def _import_snapshot(renderer, snapshot: str, *, comfy_version: str | None) -> SnapshotImport:
    """Resolve a ComfyUI Desktop snapshot into a ``definition``, in one call.
    ...
    away, and it would make ``build_not_signed_in`` depend on whether the
    snapshot happens to exist — two different failures for one cause.
    """
    ...
    try:
        ...
        renderer.error(
            code="build_spec_invalid",
            message=f"could not read the Desktop snapshot at {path}: {error}",
            hint="pass --from-snapshot <path> naming a ComfyUI Desktop snapshot JSON file",
    ...
    if not isinstance(data, dict):
        renderer.error(
            code="build_spec_invalid",
            message=f"{path} is not a ComfyUI Desktop snapshot: expected a JSON object.",
            hint="pass --from-snapshot <path> naming a ComfyUI Desktop snapshot JSON file",
            details={"path": str(path)},
        )
        raise typer.Exit(code=1)

    imported = _builder_call(renderer, lambda: client.resolve_snapshot(as_snapshot_envelope(data)))
    definition = imported.get("definition") if isinstance(imported, dict) else None
    if not isinstance(definition, dict):
        renderer.error(
            code="build_builder_error",
            message="the builder's importer returned no definition for this snapshot.",
            details={"path": str(path)},
        )
        raise typer.Exit(code=1)
    ...
    return SnapshotImport(definition=definition, advisories=advisories, path=path)
...
def _imported_payload(imported: SnapshotImport) -> dict:
    """`init`'s JSON payload for the snapshot source.
...
def _render_import_summary(renderer, imported: SnapshotImport) -> None:
    ...
    renderer.print(f"Imported {imported.path} through the builder's snapshot importer.")
    if base_version:
        renderer.print(f"ComfyUI version: {base_version}")
    else:
        renderer.warn(
...
def update_cmd(
    ...
    payload = {
        ...
        "source": "snapshot" if imported is not None else "scan",
        "models_dir": str(paths.models_dir),
        "custom_nodes_dir": str(paths.custom_nodes_dir) if paths.custom_nodes_dir.is_dir() else None,
        "dry_run": dry_run,
        "summary": summary,
...
def push_cmd(
    ...
    uploaded = _builder_call(renderer, lambda: upload_assets(preparation, client))
    ...
    if release:
        ...
        version_id, status_url = _builder_call(renderer, lambda: client.create_release(target_id, requested))
        release_summary = {"versionId": version_id, "statusUrl": status_url}
        payload["targets"] = requested
        payload["release"] = release_summary
    if renderer.is_pretty():
        renderer.success(f"Pushed build {target_id} ({uploaded} upload(s))")
        if release_summary is not None:
            renderer.print(f"  release: {release_summary['versionId']}")
...
def ls_cmd(builder_url: Annotated[str | None, _BUILDER_URL_OPT] = None):
    ...
    renderer.emit({"distributions": dists}, command="build ls")
...
def delete_cmd(
    ...
    renderer.emit({"distributionId": selected_build_id, "deleted": True}, command="build delete", changed=True)
...
def blob_ls(
    ...
    renderer.emit({"blobs": blobs}, command="build blob ls")
