...
class _ResolvedAssets:
    ...
    def payload(self) -> JsonObject:
        return {"uploaded": self.uploaded, "deduped": self.deduped, "bytes": self.bytes}
...
def _resolve_assets(plan: WorkflowAssetPlan, client: DeployAssetClient, upload_enabled: bool) -> _ResolvedAssets:
    asset_ids: dict[str, str] = {}
    results: list[AssetResolveResult] = []
    for asset in plan.assets:
        result = client.resolve_asset(AssetResolveRequest(asset.local_path, asset.file_path, upload_enabled))
...
def run_deploy(ctx: typer.Context, request: DeployRunRequest) -> None:
    ...
    try:
        ...
        assets = _resolve_assets(plan, DeployAssetClient(endpoint_origin, cloud_token), not request.no_upload)
