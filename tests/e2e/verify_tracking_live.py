...
def _send_smoketest_event(nonce: str) -> tuple[str, str]:
    ...
    # Providers are constructed lazily; force construction before filtering.
    posthog_providers = [p for p in tracking._get_providers() if isinstance(p, tracking.PostHogProvider) and p.enabled]
