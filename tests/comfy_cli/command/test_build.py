...
def test_plan_create_carries_environment_fields():
    ...
    d = build.plan_create(definition)["definition"]
...
def test_plan_create_resolver_turns_model_into_sourceuri():
    ...
    plan = build.plan_create(SCAN_DEF, resolver=resolver)
...
def test_execute_create_rejects_local_node_upload():
    ...
    with pytest.raises(NotImplementedError):
        build.execute_create(plan, client=_FakeBuilder(), name="d", locate_bytes=lambda u: Path("/x"))
...
def test_resolve_model_source_reads_annotation():
    assert build.resolve_model_source({"filename": "x", "sourceUri": "https://u"}) == "https://u"
    assert build.resolve_model_source({"filename": "x"}) is None
...
def test_update_command_reads_updated_at_before_patch(monkeypatch, tmp_path):
    ...
    build.update_cmd(build_id="d1", from_=str(d), builder_url="https://builder.test/")
...
def test_plan_create_keeps_registry_pins_out_of_the_upload_plan():
    ...
    plan = build.plan_create(REGISTRY_DEF)
...
def test_plan_create_keeps_a_blob_reference():
    ...
    priv = next(n for n in build.plan_create(REGISTRY_DEF)["definition"]["customNodes"] if n["name"] == "priv")
...
def test_plan_create_honours_a_hand_written_git_node_with_no_source_tag():
    ...
    g = next(n for n in build.plan_create(REGISTRY_DEF)["definition"]["customNodes"] if n["name"] == "gitpack")
...
def test_plan_create_carries_base_image_and_policies():
    ...
    d = build.plan_create(REGISTRY_DEF)["definition"]
...
def _execute(monkeypatch, builder, definition):
    ...
    build._create_execute(get_renderer(), definition, name="demo", builder_url=None, models_dir=None)
...
def test_update_maps_a_scan_definition_instead_of_sending_it_raw(monkeypatch):
    ...
    class FakeClient:
        def get_build(self, did):
            return {"updatedAt": "t0"}

        def update_build(self, did, definition, updated_at):
    ...
    monkeypatch.setattr(build, "_load_definition", lambda p, require_models=False: defn)
    build.update_cmd("d1", from_="ignored.json")
...
def test_update_refuses_what_it_cannot_upload(monkeypatch):
    ...
    monkeypatch.setattr(build, "_builder_client", lambda renderer, url: FakeClient())
    monkeypatch.setattr(build, "resolve_models_via_builder", lambda models, client: 0)
    defn = {
...
def test_a_definition_naming_only_blobs_goes_through_update_untouched():
    ...
    the file as written."""
    assert not build._is_scan_shaped(
        {
...
# --- from-snapshot: a Desktop export becomes a build in one call --------------
...
def test_from_snapshot_schema_matches_what_the_builder_serves():
    """The builder's SnapshotBuild answer is {build, report}, and the CLI emits
    it verbatim, so the shipped schema has to require `build`. It required the
    pre-rename `distribution` key, which no response has carried since the
    builder's public API renamed distributions to builds."""
    jsonschema = pytest.importorskip("jsonschema")
    schema_path = Path(build.__file__).parents[1] / "schemas" / "build_from_snapshot.json"
...
def test_from_snapshot_refuses_a_file_that_is_not_json(tmp_path, capsys):
    ...
    with pytest.raises(typer.Exit):
        build.from_snapshot_cmd(from_=str(bad), name="demo")
...
def test_distribution_api_module_is_a_warning_shim():
    """`comfy_cli.distribution_api` is the deprecated spelling of
    `comfy_cli.builder_api`, kept for one release: importing it warns once and
    hands back the same client classes, so an old `import` keeps working."""
    import importlib

    sys.modules.pop("comfy_cli.distribution_api", None)
    with pytest.warns(DeprecationWarning, match="comfy_cli.builder_api"):
        shim = importlib.import_module("comfy_cli.distribution_api")
    from comfy_cli import builder_api

    assert shim.BuilderClient is builder_api.BuilderClient
    assert shim.BuilderAuthError is builder_api.BuilderAuthError
...
def test_release_create_json_validates_against_extended_schema(tmp_path):
    ...
    schema_path = Path(build.__file__).parents[1] / "schemas" / "build_version_create.json"
...
def test_plan_create_reads_a_registry_pin_that_names_the_pack_once():
    ...
    plan = build.plan_create({"customNodes": [{"name": "comfyui-kjnodes", "registryVersion": "1.4.9"}]})
...
def test_from_snapshot_refuses_json_that_is_not_an_object(tmp_path, capsys, payload):
    ...
    with pytest.raises(typer.Exit):
        build.from_snapshot_cmd(from_=str(f), name="demo")
...
def _workflow_client(payload):
    ...
    client.create_build_from_workflow.return_value = payload
    return client
...
def test_from_workflow_client_posts_the_graph_to_its_own_endpoint(monkeypatch):
    ...
    assert client.create_build_from_workflow("portrait", UI_WORKFLOW)["build"]["id"] == "dist-9"
    ...
    client.create_build_from_workflow("portrait", UI_WORKFLOW, description="a portrait pipeline")
...
def test_from_workflow_accepts_the_api_export_dialect(monkeypatch, tmp_path, capsys):
    ...
    client = _workflow_client(_built({}))
    monkeypatch.setattr(build, "_builder_client", lambda renderer, url: client)
    build.from_workflow_cmd(from_=_write_workflow(tmp_path, api_export), name="portrait")
    assert client.create_build_from_workflow.call_args.args[1] == api_export
...
def test_from_workflow_names_the_extraction_and_the_update_when_no_comfy_version_is_pinned(
    ...
    monkeypatch.setattr(
        build, "_builder_client", lambda renderer, url: _workflow_client(_built({"comfyVersionRequired": True}))
    )
    build.from_workflow_cmd(from_=_write_workflow(tmp_path, UI_WORKFLOW), name="portrait")
...
def test_from_workflow_names_the_release_command_when_a_comfy_version_is_pinned(monkeypatch, tmp_path, capsys):
    monkeypatch.setattr(
        build, "_builder_client", lambda renderer, url: _workflow_client(_built({"comfyVersionRequired": False}))
    )
    build.from_workflow_cmd(from_=_write_workflow(tmp_path, UI_WORKFLOW), name="portrait")
    out = " ".join(capsys.readouterr().out.split())
...
def test_from_workflow_says_an_empty_report_checked_nothing(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(build, "_builder_client", lambda renderer, url: _workflow_client(_built({})))
    build.from_workflow_cmd(from_=_write_workflow(tmp_path, UI_WORKFLOW), name="portrait")
    out = " ".join(capsys.readouterr().out.split())
...
def test_from_workflow_refuses_to_claim_success_on_an_answer_it_cannot_read(monkeypatch, tmp_path, capsys, payload):
    ...
    monkeypatch.setattr(build, "_builder_client", lambda renderer, url: _workflow_client(payload))
    with pytest.raises(typer.Exit):
        build.from_workflow_cmd(from_=_write_workflow(tmp_path, UI_WORKFLOW), name="portrait")
    out = capsys.readouterr().out
...
def test_from_workflow_refuses_a_path_it_cannot_read(tmp_path, capsys):
    with pytest.raises(typer.Exit):
        build.from_workflow_cmd(from_=str(tmp_path), name="portrait")
    assert "build_workflow_invalid" in capsys.readouterr().out
