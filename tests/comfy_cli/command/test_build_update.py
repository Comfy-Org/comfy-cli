...
def test_base_image_is_written_into_the_definition_the_builder_receives(install: Path, spec_file: Path) -> None:
    """`--base-image` names a curated runtime by id. The builder validates the id
    on every write and the baker resolves it at build time, so carrying it in the
    definition is the whole mechanism — there is no separate API call."""
    # Given
    runner = CliRunner()

    # When
    result = _run(runner, "update", install, "-y", "--base-image", "cuda124-py312")

    # Then
    assert result.exit_code == 0, result.output
    assert _envelope(result)["data"]["definition"]["baseImage"] == "cuda124-py312"
    assert read_build_spec(spec_file)["definition"]["baseImage"] == "cuda124-py312"


def test_a_rescan_keeps_the_base_image_it_was_not_asked_to_change(install: Path, spec_file: Path) -> None:
    """A scan reports no base image at all, so `merge_definition` has to carry
    the stored one forward. Without that, every plain `update` would silently
    reset the build to the catalog default."""
    # Given
    assert _run(CliRunner(), "update", install, "-y", "--base-image", "cuda124-py312").exit_code == 0
...
def test_changing_the_base_image_shows_up_in_the_diff_the_user_confirms(install: Path, spec_file: Path) -> None:
    """The confirmation prompt is the last gate before a build's runtime moves,
    so the change has to be visible in the diff rather than applied silently."""
    # Given
    assert _run(CliRunner(), "update", install, "-y", "--base-image", "cuda124-py312").exit_code == 0

    # When
    envelope = _envelope(_run(CliRunner(), "update", install, "-y", "--base-image", "cuda128-py311"))

    # Then
    assert envelope["data"]["diff"]["baseImage"] == "changed"
