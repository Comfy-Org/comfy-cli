...
def test_plaintext_deploy_url_env_var_is_named_in_the_refusal(monkeypatch):
    # Given
    monkeypatch.setenv("COMFY_DEPLOY_URL", "http://example.com/deploy")

    # When / Then
    with pytest.raises(DeployAPIError) as exc_info:
        DeployClient(None, "jwt-token")
    assert exc_info.value.code == "deploy_insecure_url"
    assert exc_info.value.details["source"] == "COMFY_DEPLOY_URL"
...
@pytest.mark.parametrize(
    "compute_config",
    [
        pytest.param({"gpuClass": "l4", "region": "US-MO-2", "min": 5}, id="min-without-max"),
        pytest.param({"gpuClass": "l4", "region": "US-MO-2", "max": 3}, id="max-without-min"),
        pytest.param({"gpuClass": "l4", "region": "US-MO-2"}, id="neither-bound"),
    ],
)
def test_scale_accepts_a_compute_config_carrying_only_one_bound(recorder, compute_config):
    """`run_scale` sends only the bounds the flags and the stored config supply,
    so comparing against a bound synthesized here refuses `--min 5` on behalf of
    a maximum nobody set."""
    # Given
    client = DeployClient(_BASE_URL, "jwt-token")

    # When
    client.update_deployment("dep-1", compute_config)

    # Then
    assert recorder.calls[0]["body"] == {"computeConfig": compute_config}


def test_scale_still_rejects_a_min_above_an_explicit_max(recorder):
    # Given
    client = DeployClient(_BASE_URL, "jwt-token")

    # When
    with pytest.raises(DeployAPIError) as exc_info:
        client.update_deployment("dep-1", {**_COMPUTE, "min": 2, "max": 1})
...
def test_a_non_string_next_cursor_ends_pagination_instead_of_being_sent_back(monkeypatch):
    # Given
    calls: list[str] = []
    ...
    def page_request(url, target, **kwargs):
        after = urllib.parse.parse_qs(urllib.parse.urlsplit(url).query).get("after", [""])[0]
        calls.append(after)
        return 200, {"deployments": [{"id": "dep-1"}], "nextCursor": 5}

    monkeypatch.setattr("comfy_cli.deploy_api.request_json", page_request)

    # When
    yielded = list(DeployClient(_BASE_URL, "jwt-token").iter_deployments())
...
def test_a_repeated_cursor_is_reported_rather_than_walked_forever(monkeypatch):
    # Given
    calls: list[str] = []
    ...
    def page_request(url, target, **kwargs):
        after = urllib.parse.parse_qs(urllib.parse.urlsplit(url).query).get("after", [""])[0]
        calls.append(after)
        return 200, {"deployments": [{"id": "dep-1"}], "nextCursor": "stuck"}

    monkeypatch.setattr("comfy_cli.deploy_api.request_json", page_request)

    # When / Then
    with pytest.raises(DeployAPIError) as exc_info:
        list(DeployClient(_BASE_URL, "jwt-token").iter_deployments())
    assert exc_info.value.code == "deploy_server_error"
    assert exc_info.value.details["cursor"] == "stuck"
    assert calls == ["", "stuck"]


def test_an_endless_stream_of_fresh_cursors_stops_at_the_page_ceiling(monkeypatch):
    # Given
    calls: list[str] = []
    ...
    def page_request(url, target, **kwargs):
        after = urllib.parse.parse_qs(urllib.parse.urlsplit(url).query).get("after", [""])[0]
        calls.append(after)
        return 200, {"deployments": [{"id": "dep-1"}], "nextCursor": f"c{len(calls)}"}
    ...
    with pytest.raises(DeployAPIError) as exc_info:
        list(DeployClient(_BASE_URL, "jwt-token").iter_deployments())
    assert exc_info.value.details["pages"] == deploy_api._MAX_PAGES
    assert len(calls) == deploy_api._MAX_PAGES
