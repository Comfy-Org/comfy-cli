...
def _parse_input_spec(spec: Any) -> tuple[str, bool, list[Any], PortOptions, list[dict]]:
    """Returns (type_id, is_enum, enum_values, options, dynamic_options)."""
    ...
    if isinstance(first, str):
        if _is_dynamic_combo_type(first):
            ...
            return first, True, [o["key"] for o in dyn], port_opts, dyn
...
def _parse_inputs(raw: dict, order: list[str] | None, required: bool) -> list[Port]:
    ...
    for name in _ordered_names(raw, order):
        spec = raw[name]
        type_id, is_enum, enum_values, opts, dynamic_options = _parse_input_spec(spec)
        ports.append(
            Port(
                name=name,
                type=type_id,
...
def _sub_port_from_spec(name: str, sub_spec: Any, required: bool) -> Port:
    ...
    type_id, is_enum, enum_values, opts, dynamic_options = _parse_input_spec(sub_spec)
    return Port(
        name=name,
        type=type_id,
        required=required,
...
def _expand_widget_entries(m: Morphism, widgets_values: list[Any]) -> list[_WidgetEntry]:
    ...
    def emit(name: str, port: Port, owner: str | None, depth: int) -> None:
        ...
        elif port.options.control_after_generate:
...
def _dynamic_combo_default_values(
    ...
    for sub in _dynamic_combo_sub_ports(dynamic_options, selector, prefix):
        ...
        elif sub.options.control_after_generate:
            values.append("fixed")
